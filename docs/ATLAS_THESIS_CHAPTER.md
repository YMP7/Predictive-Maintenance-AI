...
| **Laptop** | Consumer Workstations / PCs | **16 acquired channels**: 5 canonical model features (`cpu_usage`, `memory_usage`, `disk_usage`, `battery_percent`, `is_charging`) + 11 physical engineering metrics (`physical_cores`, `logical_cores`, `cpu_freq_current_ghz`, `ctx_switches_per_sec`, `interrupts_per_sec`, `disk_read_mbps`, `disk_write_mbps`, `net_sent_mbps`, `net_recv_mbps`, `battery_voltage_mv`, `thermal_headroom` thermodynamic estimate) | Direct OS kernel hooks (`psutil`) | 10–20 Hz | Thermal saturation, battery wear, memory exhaustion (Category B) |
| **Mobile** | Android Smartphones | **15 acquired channels**: 5 canonical model features (`battery_level`, `battery_temp`, `battery_current`, `memory_used_percent`, `cpu_usage`) + 10 physical hardware metrics (`battery_voltage`, Bosch Sensortec BMI320 3-axis accel, vibration RMS, 3-axis gyro, AMS TCS3701 ambient light, proximity, QST QMC6308 3-axis mag) | Three-tier graceful fallback (Tier 1: Wi-Fi `Termux:API` / Lumia Web Bridge; Tier 2: USB direct ADB shell dumpsys; Tier 3: Multi-frequency simulation) | 0.2–5.0 Hz | Thermal runaway, discharge spikes, battery wear (Category B) |
